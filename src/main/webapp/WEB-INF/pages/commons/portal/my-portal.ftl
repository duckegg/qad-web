...
<div class="portlet-container kui-init-disable-sort horizontal" id="${pageId}-portlets"></div>
<div id="${pageId}-new-portlet-dialog" style="display: none">
    <form data-kui-ajax-form id="${pageId}-portlet-form" action="${base}/sample/my"
    ...
    <#--<@ui.textarea label="内容" name="content"/>-->
        <@ui.buttonGroup>
...
<script type="text/javascript">
    $(function () {
        ...
        var $page = $('#${pageId}');
        var editMode = false;
        var allPortlets = loadLocal();
        var $portletContainer = $('.portlet-container', $page);
        var $portletForm = $('#${pageId}-portlet-form');
        buildPortlets(allPortlets);
        setLayout(editMode);
        initEvent($page);
...
//                e.stopPropagation();
        ...
        function loadLocal() {
            ...
            var portlets = JSON.parse(str);
            portlets = $.isArray(portlets) ? portlets : [];
            var id = genId();
            return  _.forEach(portlets, function (p) {
        ...
        function buildOnePortlet(portlet) {
            ...
            var html = '<div class="portlet panel panel-default" data-kui-content-url="' + portlet.url + '" id="' + portletId + '">' +
