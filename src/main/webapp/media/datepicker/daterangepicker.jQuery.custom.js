...
jQuery.fn.daterangepicker = function(settings){
	...
	var options = jQuery.extend({
		presetRanges: [
			...
			{text: '今年（至今）', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: 'today' },
			{text: '今年（全年）', dateStart: function(){ var x= Date.parse('today'); x.setMonth(0); x.setDate(1); return x; }, dateEnd: function(){return new Date(new Date().getFullYear(), 11, 31);} },
			{text: '明年', dateStart: function(){ var x= Date.parse('today');x.setFullYear(x.getFullYear()+1); x.setMonth(0); x.setDate(1); return x; }, dateEnd: function(){return new Date(new Date().getFullYear()+1, 11, 31);} }
			//extras:
    ...
    // LEOLIAO 2012/05: A quick fix to instActive.inline bug in 1.8.18
