...
@param allowInput boolean, true(default): user can freely input tag, false: user can only select
@param selectedValues sequence(preferred) or string, pre-selected tag labels
@param controlName string, HTML control name, if allowInput is true, it will generate hidden controls
================================================================================
-->
...
<div class="form-group" id="${id}-group">
    ...
    <div class="controls">
        <#if allowInput>
            ...
            <#assign allTags="["+allTags+"]"/>
            <input type="text" id="${id}" value="${sequenceValues?join(',')}"
                   placeholder="请选择或输入标签"
                   class="${class}" style="${style}"/>
            <#list sequenceValues as val>
        ...
        <#--<a href="#${id}-help" data-dialog="#${id}-help"><i class="fa fa-info"></i></a>-->
        ...
        $('#${id}').select2({
            ...
            <#if allowInput>
                    .on('change', function (e) {
                        $('[name="${controlName}"]', $container).remove();
                        $.each(e.val, function (index, value) {
                            $container.append('<input type="hidden" name="${controlName}" value="' + value + '"/>');
                        });
...
@param selectedValues sequence or string, pre-selected customers
================================================================================
-->
...
<#-- Ensure sequenceValues is type of sequence -->
    <#if selectedValues?is_sequence>
        <#assign sequenceValues=selectedValues/>
    <#else>
        <#assign sequenceValues=selectedValues?eval/>
