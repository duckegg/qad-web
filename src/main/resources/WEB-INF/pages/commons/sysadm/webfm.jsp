<%--
	...
	@revision Leo Liao, 2013/06/17, modified to work with Struts2, sitemesh, bootstrap
--%>
<%@ page contentType="text/html; charset=UTF-8" %>
<%@page import="hpps.qad.base.auditor.AppAuditor,
                org.slf4j.Logger,
...
<%!
    static final Logger logger = LoggerFactory.getLogger("hpps.admin.devel.webfm");
    //FEATURES
    ...
    private static final String RESTRICT_PATH = "E:\\;/usr/opshere";
     ...
     */
    private static final String[] WIN_COMMAND_INTERPRETER = {"cmd", "/C"}; // Dos,Windows
    private static final String[] UNIX_COMMAND_INTERPRETER = {"/bin/sh", "-c"};    // Unix
    ...
    //Button names
    private static final String SAVE_AS_ZIP = "Download files as (z)ip";
    private static final String RENAME_FILE = "Rename File";
    private static final String DELETE_FILES = "Delete";
    private static final String CREATE_DIR = "New Folder";
    private static final String CREATE_FILE = "New File";
    private static final String MOVE_FILES = "Move";
    private static final String COPY_FILES = "Copy";
    private static final String LAUNCH_COMMAND = "Launch Program";
    ...
    static String startProcess(String command, String dir) throws IOException {
        ...
        comm[0] = isWindows ? WIN_COMMAND_INTERPRETER[0] : UNIX_COMMAND_INTERPRETER[0];
        comm[1] = isWindows ? WIN_COMMAND_INTERPRETER[1] : UNIX_COMMAND_INTERPRETER[1];
        comm[2] = command;
    ...
    static String dir2linkdir(String dir, String browserLink, int sortMode) {
        ...
        while (f.getParentFile() != null) {
            if (f.canRead()) {
                ...
                buf.insert(0, "<li><a href=\"" + browserLink + "?sort=" + sortMode + "&amp;dir="
                        + encPath + "\">" + conv2Html(f.getName()) + File.separator + "</a></li>");
            } else buf.insert(0, conv2Html(f.getName()) + File.separator);
            f = f.getParentFile();
        }
        if (f.canRead()) {
            String encPath = URLEncoder.encode(f.getAbsolutePath());
            buf.insert(0, "<li><a href=\"" + browserLink + "?sort=" + sortMode + "&amp;dir=" + encPath
                    + "\">" + conv2Html(f.getAbsolutePath()) + "</a></li>");
        } else
            buf.insert(0, f.getAbsolutePath());
//        buf.insert(0,"<ul class=\"breadcrumb\">").append("</ul>");
        return buf.toString();
    }
...
<%
    ...
    final String browser_name = "/admin/devel/webfm";
...
<div id="command-window">
    <title>Launch commands</title>
    <h4><%=request.getAttribute("dir")%>
    </h4>
    <%
        String ret = "";
        String command = request.getParameter("command");
        if (!command.equalsIgnoreCase("")) {
            AppAuditor.log("webfm.command", command);
            ret = startProcess(command, (String) request.getAttribute("dir"));
        }
    %>
    <div>
        <pre class="well" style="height: 25em;overflow: auto"><%=ret%></pre>
    </div>
    <form action="<%=browser_name%>" method="POST" class="form-inline" data-ajax-form
          data-kui-target="#command-window">
        <input type="hidden" name="dir" value="<%= request.getAttribute("dir")%>">
        <input type="text" id="command-input" name="command" class="input-xxlarge"
               placeholder="Enter your command" value="<%=request.getParameter("command")%>">
        <input class="btn btn-primary"
               type="Submit" name="Submit"
               value="Launch">
        <input type="hidden" name="sort" value="<%=request.getParameter("sort")%>">
        <a href="<%=browser_name%>?dir=<%= request.getAttribute("dir")%>&sort=<%=request.getParameter("sort")%>">Cancel</a>
    </form>
...
<table class="filelist table table-bordered">
    <th>Name</th>
    ...
    %>
    <tr class="mouseout">
        <td><%=conv2Html(entry.getName())%>
        </td>
        <td><%=convertFileSize(entry.getSize())%>
        </td>
        <td>
    ...
    try {
        ...
        if (!isAllowed(new File((String) ht.get("dir")), false)) {
            ...
            if (info != null && info.aborted) {
                ...
                AppAuditor.log("webfm.upload", dest);
                if (!f.renameTo(dest)) {
...
<form action="<%=browser_name%>" method="Post">
    <%
        ...
        dir_view = false;
    %>
    <textarea name="text" class="col-md-10"
              cols="<%=EDITFIELD_COLS%>"
              rows="<%=EDITFIELD_ROWS%>" <%=disable%>><%=sb.toString()%>
    </textarea>
    <input type="hidden" name="nfile" value="<%= request.getParameter("editfile")%>">
    <input type="hidden" name="sort" value="<%=request.getParameter("sort")%>">

    <div>
        <label class="radio-inline"><input type="radio" name="lineformat"
                                           value="dos" <%= dos?"checked":""%>>Ms-Dos/Windows</label>
        <label class="radio-inline"><input type="radio" name="lineformat"
                                           value="unix" <%= dos?"":"checked"%>>Unix</label>
    </div>
    <div>
        <label class="checkbox-inline"><input type="checkbox" name="Backup" checked>Write
            backup</label>
        <input type="text" name="new_name" value="<%=ef.getName()%>"
               title="Enter the new filename">
        <button type="Submit" name="Submit" value="Save" class="btn">Save</button>
        <a class="btn"
           href="<%=browser_name%>?sort=<%=request.getParameter("sort")%>&dir=<%=ef.getParent()%>">Cancel</a>
    </div>
</form>
<hr>
<%
    ...
    // Save or cancel the edited file
        ...
        if (request.getParameter("Submit").equals("Save")) {
            ...
            if (new_f.exists() && !new_f.canWrite())
                ...
                outs.flush();
                outs.close();
                AppAuditor.log("webfm.edit",new_f);
            }
        }
        request.setAttribute("dir", f.getParent());
    }
    //Unpack file to the current directory without overwriting
        ...
        //GZip
            ...
            try {
                byte buffer[] = new byte[0xffff];
                copyStreams(new GZIPInputStream(new FileInputStream(f)),
                        new FileOutputStream(newName), buffer);
                AppAuditor.log("webfm.unpack",f);
            } catch (IOException ex) {
        ...
        //Else try Zip
            ...
            try {
                ...
                if (!error) {
                    ...
                    AppAuditor.log("webfm.unpack", f.getName());
                }
            } catch (ZipException ex) {
    ...
    // Delete Files
        ...
        Vector v = expandFileList(selfiles, true);
        boolean error = false;
        //delete backwards
        ...
        if ((!error) && (v.size() > 1)) {
            ...
            AppAuditor.log("webfm.delete", Arrays.asList(selfiles));
    ...
    // Create Directory
        ...
        if (!isAllowed(new File(new_dir), true)) {
            ...
            AppAuditor.log("webfm.create", new_dir);
        } else {
    ...
    // Create a new empty file
        ...
        // Test, if file_name is empty
            ...
            if (new File(new_file).createNewFile()) {
                ...
                AppAuditor.log("webfm.create", new_file);
            } else {
        ...
        // 1) Zero Files selected
            ...
            // Test, if file_name is empty
                ...
                if (!f.canWrite() || !f.renameTo(new File(new_file.trim()))) {
                    ...
                    request.setAttribute("message", "Renamed file " + ((File) v.get(0)).getName() + " to " + new_file);
                    AppAuditor.log("webfm.rename", f + " to " + new_file);
                }
            } else {
                request.setAttribute("error", "Error: \"" + new_file_name + "\" is not a valid filename");
            }
        }
    }
    // Move selected file(s)
        ...
        if (!isAllowed(new File(new_dir), false)) {
            ...
            if ((!error) && (v.size() > 1)) {
                ...
                AppAuditor.log("webfm.move", Arrays.asList(selfiles) + " to " + new_dir);
...
//        if (fname != "")
//            $('#upload-monitor').load(URL + "?first&uplMonitor=" + encodeURIComponent(fname));
        if (fname != "")
...
%>
Filter: <input name="filt" onKeypress="event.cancelBubble=true;" onkeyup="filter(this)"
               type="text">
<input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
...
<div class="btn-toolbar">
    <% if (!READ_ONLY) {%>
    <input type="hidden" id="text_Dir" name="cr_dir">
    <script type="text/javascript">
        function inputFilename(msg) {
            var filename = prompt(msg);
            if (filename == null)
                return false;
            $('#text_Dir').val(filename);
            return true;
        }
    </script>
    <div class="btn-group">
        <button title="Create a new directory with the given name" class="btn" id="but_NDi"
                type="Submit" name="Submit" value="<%=CREATE_DIR%>"
                onclick="return inputFilename('Input directory name');"><i
                class="fa fa-folder-o"></i></button>
        <button title="Create a new empty file with the given name" class="btn" id="but_NFi"
                type="Submit" name="Submit" value="<%=CREATE_FILE%>"
                onclick="return inputFilename('Input file name');"><i class="fa fa-file-o"></i>
        </button>
        <button title="Move selected files and directories to the entered path" id="but_Mov"
                class="btn" type="Submit" name="Submit" value="<%=MOVE_FILES%>"
                onclick="return inputFilename('Input destination path');"><i
                class="fa fa-random"></i></button>
        <button title="Copy selected files and directories to the entered path" id="but_Cop"
                class="btn" type="Submit" name="Submit" value="<%=COPY_FILES%>"
                onclick="return inputFilename('Input destination path');"><i
                class="fa fa-copy"></i></button>
    </div>
    <button title="Delete all selected files and directories incl. subdirs"
            class="btn btn-danger"
            id="but_Del" type="Submit" name="Submit" value="<%=DELETE_FILES%>"
            onclick="return confirm('Do you really want to delete the entries?')"><i
            class="fa fa-trash-o"></i> Delete
    </button>
    <button title="Rename selected file or directory to the entered name" id="but_Ren"
            class="btn"
            onclick="return inputFilename('Input new name')"
            type="Submit" name="Submit" value="<%=RENAME_FILE%>"><i class="fa fa-tag"></i>
    </button>
    <% } %>
    <div class="btn-group">
        <button title="Download selected files and directories as zip file" class="btn"
                id="but_Zip" type="Submit" name="Submit" value="<%=SAVE_AS_ZIP%>"><i
                class="fa fa-download"></i>
        </button>
    </div>
    <% if (NATIVE_COMMANDS) {%>
    <a class="btn btn-info"
       href="<%=browser_name%>?dir=<%=request.getAttribute("dir")%>&sort=<%=sortMode%>&command"><i
            class="fa fa-plane"></i> Launch
        command</a>
    <% }%>
</div>
<div class="btn-toolbar">
    <div class="btn-group">
        <% File[] entry = File.listRoots();// Output the Root-Dirs, without FORBIDDEN_DRIVES
            for (int i = 0; i < entry.length; i++) {
                boolean forbidden = false;
                for (int j = 0; j < FORBIDDEN_DRIVES.length; j++) {
                    if (entry[i].getAbsolutePath().toLowerCase().equals(FORBIDDEN_DRIVES[j]))
                        forbidden = true;
                }
                if (!forbidden) {
        ...
        %>
        <a class="btn" href="<%=browser_name%>?sort=<%=sortMode%>&dir=<%=name%>"><%=buf%>
        </a>
        <% }
        }%>
    </div>
    <% if (f.getParent() != null) { // Output the parent directory link ".."    %>
    <div class="btn-group">
        <a class="btn btn-info" title="Up folder"
           href="<%= browser_name%>?sort=<%=sortMode%>&dir=<%=URLEncoder.encode(f.getParent())%>"><%=FOL_IMG%>
            <i class="fa fa-arrow-up"></i> </a>
    </div>
    <% } %>
</div>
<table id="filetable" class="filelist table table-bordered table-hover">
    <% for (int i = 0; i < sort.length; i++) {
        if (sort[i] == sortMode)
            sort[i] = -sort[i];
    }%>
    <tr>
        <th><input type="checkbox" name="selall" title="Check all"
                   onClick="AllFiles(this.form)"></th>
        <th title="Sort files by name" align=left><a href="<%=cmd%>&sort=<%=sort[0]%>">Name</a></th>
        <th title="Sort files by size" align="right"><a href="<%=cmd%>&sort=<%=sort[1]%>">Size</a>
        </th>
        <th title="Sort files by type" align="center"><a href="<%=cmd%>&sort=<%=sort[3]%>">Type</a>
        </th>
        <th title="Sort files by date" align="left"><a href="<%=cmd%>&sort=<%=sort[2]%>">Date</a>
        </th>
        <th>&nbsp;</th>
        <%if (!READ_ONLY) {%>
        <th>&nbsp;</th>
        ...
        if (entry != null && entry.length > 0) {
            ...
            for (int i = 0; i < entry.length; i++) {
                ...
                if (entry[i].isDirectory()) {
                    ...
                    if (entry[i].canRead()) {
                        dlink = ahref + "downfile=" + name + "\" data-pjax-disabled><i class='fa fa-download'></i> Download</a>";
                        ...
                        if (entry[i].canWrite()) { // The file can be edited
                            ...
                            if (isPacked(name, true)) elink = ahref + "unpackfile=" + name
                                    + "\"><i class='fa fa-upload-alt'></i> Unpack</a>";
                            else
                                elink = ahref + "editfile=" + name + "\"><i class='fa fa-edit'></i> Edit</a>";
                        } else { // If the file cannot be edited
                            //If it is a zip or jar File you can unpack it
                            if (isPacked(name, true)) elink = ahref + "unpackfile=" + name
                                    + "\"><i class='fa fa-upload-alt'></i> Unpack</a>";
                            else
                                elink = ahref + "editfile=" + name + "\"><i class='fa fa-eye-open'></i> View</a>";
                        }
                    } else {
                        link = buf;
                    }
...
<ul class="breadcrumb">
    <li title="<%=totalSize%> bytes">
        <%=fileCount%> files, <%=convertFileSize(totalSize)%> in
    </li>
    <%= dir2linkdir((String) request.getAttribute("dir"), browser_name, sortMode)%>
</ul>
<div>
    <% if (ALLOW_UPLOAD) { %>
    <form action="<%= browser_name%><%--?_nos2multipartparser--%>"
          enctype="multipart/form-data" method="POST">
        <input type="hidden" name="dir" value="<%=request.getAttribute("dir")%>">
        <input type="hidden" name="sort" value="<%=sortMode%>">
        <input type="file" class="textfield" onKeypress="event.cancelBubble=true;" name="myFile">
        <input title="Upload selected file to the current directory" type="Submit"
               class="btn" name="Submit" value="<%=UPLOAD_FILES%>"
               onClick="popUp('<%= browser_name%>')">
    </form>
